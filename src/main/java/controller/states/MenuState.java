...
public class MenuState extends State {
	...
	public void tick() {
		...
		if (State.loadGame || State.newGame) // If load or new game true then it will change to gameState so release
												// menu memory and changes state
		{
	 ...
	 * 
	 * @param graph
	 *            Used to render the menu in the canvas
	 */
	@Override
	...
	/**
	 * Initialize the screen and buttons images and positions of the menu screen
	 */
	...
	/**
	 * Initialize the screen and buttons images of the load option from menu screen
	 */
	...
	/**
	 * Initialize the screen and buttons options when a game is selected
	 */
	...
	/**
	 * Load in memory the image of the load/delete/cancel options when a game is
	 * selected
	 */
	private void initGameSelectedButtons() {
		...
		// Load a saved game
		loadSubMenu.getButtons().add(new UIButton((int) (50 * game.getScale()), (int) (300 * game.getScale()),
				buttonWidth, buttonHeight, loadSaveButton, -1, new ButtonAction() {
					@Override
					public void action() {
						State.loadGame = true; // Tells gameState to load a game
						game.getKeyboard().mESC = true; // Set esc true to release memory from this screen (GameSelected
														// screen)
					}
				}));
		...
		// Delete a saved game
		loadSubMenu.getButtons().add(new UIButton((int) (50 * game.getScale()), (int) (430 * game.getScale()),
				buttonWidth, buttonHeight, deleteSaveButton, -1, new ButtonAction() {
					@Override
					public void action() {
						try {
							DeleteGame.Delete(State.savedGames.get(lastButtonIndex).split(" ")[0]); // Get the name of
																									// the button
																									// pressed and
																									// removes from
																									// database
		...
		loadSubMenu.getButtons().add(new UIButton((int) (50 * game.getScale()), (int) (550 * game.getScale()),
				buttonWidth, buttonHeight, cancelButton, -1, new ButtonAction() {
					@Override
					public void action() {
						State.savedGames.clear(); // Clear database name loaded
						State.savedGames = null;
	...
	/**
	 * Load in memory all buttons of the load screen showing all saved games inside
	 * and load the scroll screen
	 * 
	 */
	private void initLoadScreenButtons() {
		...
		// Init load screen
		loadScreen = new UIScrollScreen(loadScreenImage, (int) (31 * game.getScale()), (int) (132 * game.getScale()),
				(int) (loadScreenImage.getWidth() * game.getScale()),
				(int) (loadScreenImage.getHeight() * game.getScale()), scrollSpeed);
		...
		for (int i = 0, accumulator = (int) loadScreen.getScreen().getY(); (int) i < savedGames.size(); i++) { // Accumulator
																												// controls
																												// the
																												// button
																												// position
			...
			text[0] = new Text("SaveGame " + (i + 1) + " - " + split[split.length - 1], font, Color.black,
					(int) (buttonX - (25 * game.getScale()) + buttonWidth / 4), accumulator + (int) (buttonHeight / 2));
			text[1] = new Text("SaveGame " + (i + 1) + " - " + split[split.length - 1], font, Color.white,
					(int) (buttonX - (25 * game.getScale()) + buttonWidth / 4), accumulator + (int) (buttonHeight / 2));

			loadScreen.getButtons().add(new UIButton((int) buttonX, accumulator, (int) (buttonWidth),
					(int) buttonHeight, loadButton, i, text, new ButtonAction() {
						public void action() {
	...
	/**
	 * Load in memory all images to render the menu of the game
	 * 
	 * @throws Exception
	 *             throw from database when the game fail`s to load
	 */
	private void initMenuButtons() throws Exception {
		 ...
		 * game.width to centralize the button and the game.height to control the y
		 * position on the screen for every button a Button action is defined when
		 * passing the argument, this way is possible to program the button when
		 * creating it
		 */
		...
		menuButtons.getButtons()
				.add(new UIButton((int) ((game.getWidth() / 2) - (buttonWidth / 2)),
						(int) ((game.getHeight() - game.getHeight() / 3) + buttonHeight), (int) (buttonWidth),
						(int) buttonHeight, buttonLoadGame, -1, new ButtonAction() {
							public void action() {
		...
		menuButtons.getButtons()
				.add(new UIButton((int) ((game.getWidth() / 2) - (buttonWidth / 2)),
						(int) ((game.getHeight() - game.getHeight() / 3)), (int) (buttonWidth), (int) (buttonHeight),
						buttonNewGame, -1, new ButtonAction() {
							public void action() {
								State.newGame = true;
							}
						}));
